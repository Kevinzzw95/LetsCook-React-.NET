...
class XhsMemoryStoreImplement(AbstractStore):
    ...
    def snapshot(cls) -> Dict[str, List[Dict[str, Any]]]:
        return {
            "contents": [dict(item) for item in cls._contents],
            "comments": [dict(item) for item in cls._comments],
            "creators": [dict(item) for item in cls._creators],
        }

    async def store_content(self, content_item: Dict):
        self.__class__._contents.append(dict(content_item))

    async def store_comment(self, comment_item: Dict):
        self.__class__._comments.append(dict(comment_item))

    async def store_creator(self, creator_item: Dict):
        self.__class__._creators.append(dict(creator_item))
